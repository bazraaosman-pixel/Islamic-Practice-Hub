...
export default function AdhkarScreen() {
  ...
  return (
    <ScreenShell>
      ...
      <View style={[styles.todayCard, { backgroundColor: colors.softTeal, borderColor: colors.border, flexDirection: isArabic ? 'row-reverse' : 'row' }]}>
        <View style={[styles.todayCopy, { alignItems: isArabic ? 'flex-end' : 'flex-start' }]}><Text style={[styles.todayEyebrow, { color: colors.primary, textAlign: isArabic ? 'right' : 'left', writingDirection: isArabic ? 'rtl' : 'ltr' }]}>{text('ورد اليوم', "Today's wird")}</Text><Text style={[styles.todayTitle, { color: colors.foreground, textAlign: isArabic ? 'right' : 'left', writingDirection: isArabic ? 'rtl' : 'ltr' }]}>{text('خطوات صغيرة، أثر كبير', 'Small steps, lasting impact')}</Text><Text style={[styles.todaySubtitle, { color: colors.mutedForeground, textAlign: isArabic ? 'right' : 'left', writingDirection: isArabic ? 'rtl' : 'ltr' }]}>{text(`أكملت ${completed} من 10 أذكار`, `${completed} of 10 supplications completed`)}</Text></View>
