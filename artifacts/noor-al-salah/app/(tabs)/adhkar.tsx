...
import { remembranceCategories } from '@/lib/prayerData';
import { router } from 'expo-router';

export default function AdhkarScreen() {
  const colors = useColors();
  const [selected, setSelected] = useState('morning');
  const selectedCategory = remembranceCategories.find((category) => category.id === selected) ?? remembranceCategories[0];
  ...
  return (
    <ScreenShell>
      ...
      <View style={styles.categoryGrid}>{remembranceCategories.map((category, index) => <Pressable key={category.id} testID={`adhkar-${category.id}`} onPress={() => setSelected(category.id)} style={({ pressed }) => [styles.categoryCard, { backgroundColor: selected === category.id ? colors.primary : colors.card, borderColor: selected === category.id ? colors.primary : colors.border }, pressed && { opacity: 0.78 }]}><View style={[styles.categoryIcon, { backgroundColor: selected === category.id ? 'rgba(255,255,255,0.14)' : colors.muted }]}><Feather name={category.icon === 'heart' ? 'heart' : category.icon === 'sunrise' ? 'sunrise' : 'moon'} size={17} color={selected === category.id ? colors.cream : [colors.primary, colors.gold, '#a05e57', '#7d78a5'][index]} /></View><Text style={[styles.categoryArabic, { color: selected === category.id ? colors.cream : colors.foreground }]}>{category.arabic}</Text><Text style={[styles.categoryEnglish, { color: selected === category.id ? colors.heroMuted : colors.mutedForeground }]}>{category.count}</Text></Pressable>)}</View>
      <View style={styles.sectionBlock}><SectionHeading title={selectedCategory.arabic} action="عرض الكل" /><View style={[styles.reminderCard, { backgroundColor: colors.card, borderColor: colors.border }]}><View style={[styles.reminderQuote, { backgroundColor: colors.softGold }]}><Text style={[styles.quoteMark, { color: colors.gold }]}>“</Text></View><View style={styles.reminderCopy}><Text style={[styles.reminderTitle, { color: colors.foreground }]}>أذكار موثقة بعناية</Text><Text style={[styles.reminderBody, { color: colors.mutedForeground }]}>سيتم تحميل النصوص العربية والترجمات من مصادر موثوقة، مع عداد تفاعلي لكل ذكر.</Text></View><Pressable testID="complete-adhkar" onPress={() => setCompleted((current) => Math.min(current + 1, 10))} style={({ pressed }) => [styles.completeButton, { backgroundColor: colors.primary }, pressed && { opacity: 0.75 }]}><Feather name="check" size={17} color={colors.primaryForeground} /></Pressable></View></View>
