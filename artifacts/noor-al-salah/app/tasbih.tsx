...
export default function TasbihScreen() {
  ...
  return (
    <ScreenShell scroll={false} style={styles.screen}>
       <View style={[styles.topBar, { flexDirection: isArabic ? 'row' : 'row-reverse' }]}><Pressable testID="tasbih-back" accessibilityLabel={text('العودة', 'Back')} onPress={() => router.back()} style={[styles.backButton, { backgroundColor: colors.card, borderColor: colors.border }]}><Feather name={isArabic ? 'arrow-right' : 'arrow-left'} size={19} color={colors.foreground} /></Pressable><View style={[styles.titleCopy, { alignItems: isArabic ? 'flex-end' : 'flex-start' }]}><Text style={[styles.eyebrow, { color: colors.primary, textAlign: isArabic ? 'right' : 'left' }]}>{text('ذكرٌ يطمئن القلب', 'Remembrance that calms the heart')}</Text><Text style={[styles.title, { color: colors.foreground, textAlign: isArabic ? 'right' : 'left' }]}>{text('المسبحة', 'Tasbeeh')}</Text></View><View style={{ width: 42 }} /></View>
        <View style={styles.counterArea}><Pressable testID="tasbih-counter" accessibilityRole="button" accessibilityLabel={text('زيادة عداد المسبحة', 'Increase tasbeeh counter')} onPress={increment} style={({ pressed }) => [styles.outerCircle, { borderColor: colors.border }, pressed && styles.counterPressed]}><View pointerEvents="none" style={[styles.innerCircle, { backgroundColor: colors.softTeal, borderColor: colors.primary }]}><Text style={[styles.count, { color: colors.primary }]}>{count}</Text><Text style={[styles.target, { color: colors.mutedForeground }]}>{isArabic ? `من ${target}` : `of ${target}`}</Text><Text style={[styles.tapLabel, { color: colors.primary }]}>{text('اضغط هنا للعد', 'Tap to count')}</Text></View></Pressable><Pressable testID="tasbih-dhikr" accessibilityLabel={text('تغيير الذكر', 'Change remembrance')} onPress={() => { const options = ['سبحان الله', 'الحمد لله', 'الله أكبر']; const next = options[(options.indexOf(dhikr) + 1) % options.length]; setDhikr(next); AsyncStorage.setItem('@noor-al-salah/tasbih-dhikr', next); Haptics.selectionAsync().catch(() => undefined); }}><Text style={[styles.dhikrLabel, { color: colors.foreground }]}>{dhikr}</Text></Pressable><Text style={[styles.instruction, { color: colors.mutedForeground }]}>{isArabic ? `مجموع اليوم: ${daily}` : `Today: ${daily}`}</Text></View>
         <View style={styles.controls}><Pressable testID="tasbih-reset" onPress={reset} style={[styles.resetButton, { borderColor: colors.border, backgroundColor: colors.card }]}><Feather name="rotate-ccw" size={17} color={colors.mutedForeground} /><Text style={[styles.resetText, { color: colors.foreground }]}>{text('تصفير الجلسة', 'Reset session')}</Text></Pressable><Pressable testID="tasbih-increment" accessibilityLabel={text('زيادة العداد', 'Increase counter')} onPress={increment} style={({ pressed }) => [styles.addButton, { backgroundColor: colors.primary }, pressed && { transform: [{ scale: 0.97 }] }]}><Feather name="plus" size={29} color={colors.primaryForeground} /></Pressable><Pressable testID="tasbih-target" onPress={() => { const next = target === 33 ? 99 : 33; const clamped = Math.min(countRef.current, next); countRef.current = clamped; setCount(clamped); setTarget(next); AsyncStorage.multiSet([['@noor-al-salah/tasbih-target', String(next)], ['@noor-al-salah/tasbih', String(clamped)]]); Haptics.selectionAsync().catch(() => undefined); }} style={[styles.targetButton, { borderColor: colors.border, backgroundColor: colors.card }]}><Feather name="hash" size={17} color={colors.mutedForeground} /><Text style={[styles.resetText, { color: colors.foreground }]}>{text(`الهدف ${target}`, `Target ${target}`)}</Text></Pressable></View>
         <Pressable testID="tasbih-reset-daily" onPress={resetDaily} style={styles.dailyReset}><Feather name="trash-2" size={13} color={colors.mutedForeground} /><Text style={[styles.dailyResetText, { color: colors.mutedForeground }]}>{text('تصفير مجموع اليوم', 'Reset today’s total')}</Text></Pressable>
        <View style={[styles.savedNote, { backgroundColor: colors.softGold }]}><Feather name="check-circle" size={16} color={colors.accentForeground} /><Text style={[styles.savedText, { color: colors.accentForeground }]}>{text('يتم حفظ تقدمك تلقائياً على جهازك', 'Your progress is saved automatically on this device')}</Text></View>
    </ScreenShell>
  );
}
